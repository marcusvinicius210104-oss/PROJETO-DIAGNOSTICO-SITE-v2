...
exports.handler = async (event) => {
  ...
  if (event.httpMethod === 'OPTIONS') return { statusCode: 200, headers: { 'Access-Control-Allow-Origin': '*', 'Access-Control-Allow-Headers': 'Content-Type', 'Access-Control-Allow-Methods': 'GET, OPTIONS' }, body: '' };
  ...
  try {
    ...
    if (!id) return { statusCode: 400, body: JSON.stringify({ error: 'ID nao informado' }) };

    const response = await fetch(`https://api.abacatepay.com/v1/pixQrCode/check?id=${id}`, { headers: { 'Authorization': `Bearer ${ABACATE_KEY}` } });
